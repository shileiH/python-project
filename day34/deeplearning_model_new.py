    ...
    with tqdm(total=num_epochs * len(train_loader)) as pbar:
        for epoch_id in range(num_epochs):
            ...
            for datas, labels in train_loader:
                ...
                global_step += 1
                pbar.update(1)
                pbar.set_postfix({"epoch": epoch_id, "loss": f"{loss_value:.4f}", "acc": f"{acc:.2f}%","val_loss": f"{val_loss:.4f}","val_acc": f"{val_acc:.2f}%"})
...
def train_two_classification_model(
        model,
        train_loader,
        val_loader,
        criterion,
    ...
    with tqdm(total=num_epochs * len(train_loader)) as pbar:
        for epoch_id in range(num_epochs):
            ...
            for datas, labels in train_loader:
                ...
                # 评估
                if global_step % eval_step == 0:
                    val_acc, val_loss = evaluate_two_classification_model(model, val_loader, device, criterion)
                    record_dict["val"].append({
                        "loss": val_loss, "acc": val_acc, "step": global_step
                    })
                    model.train()  # 切换回训练集模式
...
def evaluate_two_classification_model(model, data_loader, device, criterion):
    """
    评估模型在给定分类数据集上的准确率和损失
    ...
    model.eval()  # 设置为评估模式
    correct = 0  # 预测正确样本数
    total = len(data_loader.dataset)  # 直接从数据加载器获取总样本数
    running_loss = 0.0  # 总损失
    ...
    with torch.no_grad():  # 不计算梯度
        for images, labels in data_loader:
            images, labels = images.to(device), labels.to(device)
            outputs = model(images)
            # 二分类问题：输出大于0为1类，否则为0类
            predicted = (outputs.detach() > 0).int()  # 将输出转换为0/1分类，并分离计算图
            correct += (predicted == labels).sum().item()  # (predicted == labels).sum().item()返回预测正确的标签的个数
